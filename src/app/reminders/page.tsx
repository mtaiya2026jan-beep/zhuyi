...
const REMINDER_TYPES=[
  ...
  {id:"family_change",label:"家庭成員變更",icon:"👨‍👩‍👧",desc:"新增/遷出家庭成員後30天內必須申報，提前15/7/1天提醒",days:[15,7,1]},
  {id:"lease_expiry",label:"租約到期",icon:"📋",desc:"租約到期提醒，提前60/30天提醒",days:[60,30]},
  {id:"move_eligible",label:"換房資格提醒",icon:"🏠",desc:"入住滿11個月可申請換房，提前提醒",days:[0]},
...
export default function RemindersPage(){
  ...
  const selectedType=REMINDER_TYPES.find(r=>r.id===type);
  ...
  async function handleSubmit(){
    if(!form.email||!form.deadline||!type){
      setError("請填寫所有必填項");
      return;
    }
    setLoading(true);
  ...
  if(done) return(
    <div style={{fontFamily:"PingFang TC,sans-serif",minHeight:"100vh",background:"#F7F8FA",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{background:"#fff",borderRadius:20,padding:"40px 32px",maxWidth:480,width:"100%",textAlign:"center",boxShadow:"0 4px 24px rgba(0,0,0,0.08)"}}>
        ...
        <div style={{fontSize:14,color:"#5A6A8A",lineHeight:1.8,marginBottom:24}}>
          我們將在截止日前發送提醒郵件到<br/>
    ...
    <div style={{fontFamily:"PingFang TC,sans-serif",minHeight:"100vh",background:"#F7F8FA"}}>
      ...
      <div style={{maxWidth:640,margin:"0 auto",padding:"24px 16px 60px"}}>
        ...
        <div style={{background:"#fff",borderRadius:20,padding:"24px",boxShadow:"0 2px 12px rgba(0,0,0,0.06)",marginBottom:16}}>
          <div style={{fontSize:15,fontWeight:700,color:"#1A2B4A",marginBottom:16}}>第一步：選擇提醒類型</div>
        ...
        {type&&(
          <div style={{background:"#fff",borderRadius:20,padding:"24px",boxShadow:"0 2px 12px rgba(0,0,0,0.06)",marginBottom:16}}>
            <div style={{fontSize:15,fontWeight:700,color:"#1A2B4A",marginBottom:16}}>第二步：填寫提醒資料</div>
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              <div>
                <div style={{fontSize:13,color:"#5A6A8A",marginBottom:6}}>姓名（選填）</div>
                <input style={inp} placeholder="你的中文名" value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/>
              </div>
              <div>
              ...
              <div>
                <div style={{fontSize:13,color:"#5A6A8A",marginBottom:6}}>
                  {type==="income_change"?"收入變更日期":"截止日期"} <span style={{color:"#E84A4A"}}>*</span>
                </div>
              ...
              <div>
                ...
                <textarea style={{...inp,height:80,resize:"none"}} placeholder="例如：NYCHA案號、房源地址等" value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})}/>
              </div>
